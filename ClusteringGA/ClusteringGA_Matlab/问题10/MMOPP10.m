...
for i = 1:1:PN
    ...
    %%%%%%%%%%%%%%%问题1%%%%%%%%%%%%%%%%%%%%%%%
    [f1,f2,f3,f4,f5,f6,f7] = Fitness6(len,chrom,F,Fit_List1,InterNum);
    POP(i).c = chrom;
    POP(i).l = len;
    POP(i).fitness1 = f1;
    POP(i).fitness2 = f2;
...
while (length(POP)<PN)
    ...
    for i = n+1:1:PN
        [chrom,len] = Initialization(Relation,start,goal);
        %%%%%%%%%%%%%%%问题1%%%%%%%%%%%%%%%%%%%%%%%
        [f1,f2,f3,f4,f5] = Fitness5(len,chrom,F,Fit_List1,InterNum);
        POP(i).c = chrom;
        POP(i).l = len;
        POP(i).fitness1 = f1;
        POP(i).fitness2 = f2;
...
%         ss=length(cat(1,P{1,i}.l));
        ...
        for j=1:ss
            ...
            [f1,f2,f3,f4,f5,f6,f7] = Fitness6(child(j).l,child(j).c,F,Fit_List1,InterNum);
            child(j).fitness1 = f1;
            child(j).fitness2 = f2;
            child(j).fitness3 = f3;
            child(j).fitness4 = f4;
...
Pobjs=[POP.fitness1;POP.fitness2;POP.fitness3;POP.fitness4;POP.fitness5;POP.fitness6;POP.fitness7]';
[FrontNo,MaxFNo]=NDSort(Pobjs,size(POP,2));
Pareto=POP(find(FrontNo==1));
Pareto=clearing(Pareto);
save('Pareto10.mat','Pareto')
toc
...
save('Problem8_Path_coordinate.mat','Path_coordinate');

...
for i=1:len
    ...
    C(i,m+1:m+7)=[pop(i).fitness1,pop(i).fitness2,pop(i).fitness3,pop(i).fitness4,pop(i).fitness5,pop(i).fitness6,pop(i).fitness7];
    C1(i,1)=pop(i).l;
end
D=C(:,1:m);
...
for i=1:size(cluster,1)
    ...
    for j=1:num_cluster
        ...
        S(j).fitness6=C(row,m+6);
        S(j).fitness7=C(row,m+7);
    end
    P{1,i}=S;
    N(1,i)=num_cluster;
end

function [cluster,center]=APClustering(x)           %清除所有变量，关闭所有窗口，  清除命令窗口的内容     
% x=rand(20,2);
% % x=[1,0;                                   %定义一个矩阵
% %     1,1;
% %     0,1;
...
%   set(h,'Color',col,'MarkerFaceColor',col);
  ...
  center=[center;[xi1(1),xi2(1)]];
  tt=tt+1;
%   line([x(ii,1),xi1]',[x(ii,2),xi2]','Color',col);
  else
%         h=plot3(x(ii,1),x(ii,2),x(ii,3),'o');
%         hold on;
%         col=rand(1,3);
%         set(h,'Color',col,'MarkerFaceColor',col);
        xi1=x(i,1)*ones(size(ii));
        xi2=x(i,2)*ones(size(ii));
        xi3=x(i,3)*ones(size(ii));
        center=[center;[xi1(1),xi2(1),xi3(1)]];
        tt=tt+1;

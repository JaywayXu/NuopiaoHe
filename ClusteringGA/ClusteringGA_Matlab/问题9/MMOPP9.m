...
%         ss=length(cat(1,P{1,i}.l));
        ...
        for j=1:ss
            ...
            [f1,f2,f3,f4,f5] = Fitness5(child(j).l,child(j).c,F,Fit_List1,InterNum);
            child(j).fitness1 = f1;
            child(j).fitness2 = f2;
            child(j).fitness3 = f3;
            child(j).fitness4 = f4;
...
Pobjs=[POP.fitness1;POP.fitness2;POP.fitness3;POP.fitness4;POP.fitness5]';
[FrontNo,MaxFNo]=NDSort(Pobjs,size(POP,2));
Pareto=POP(find(FrontNo==1));
Pareto=clearing(Pareto);
toc
save('Pareto9.mat','Pareto')

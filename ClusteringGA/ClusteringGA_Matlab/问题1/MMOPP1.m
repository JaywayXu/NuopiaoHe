...
for i = 1:1:PN
    ...
    %%%%%%%%%%%%%%%问题1%%%%%%%%%%%%%%%%%%%%%%%
    [f1,f2] = Fitness(len,chrom,RedInter,Fit_List1,Fit_List2);
    POP(i).c = chrom;
    POP(i).l = len;
    POP(i).fitness1 = f1;
    POP(i).fitness2 = f2;
...
%         ss=length(cat(1,P{1,i}.l));
        ...
        for j=1:ss
            ...
            [f1,f2] = Fitness(child(j).l,child(j).c,RedInter,Fit_List1,Fit_List2);
            child(j).fitness1 = f1;
            child(j).fitness2 = f2;
            POP=[POP,child(j)];
        end 
...
save('Problem1_Path_coordinate.mat','Path_coordinate');
